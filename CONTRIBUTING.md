...
M3.1–M3.3, M4.1 generation service orchestration, M4.2 Gemini plan proposal adapter, M4.3 deterministic example catalog, M4.4 canonical run manifest publication, and M4.5 strict generation stdio IPC are implemented and verified; Milestone 4 is complete. Milestone 5.1 batch contracts/models/schemas, Milestone 5.2 sequential batch execution infrastructure (FR-15), and Milestone 5.3 batch filesystem & source-integrity safety boundary (FR-16) are implemented and verified; M5.4+ format handlers, manifest publication, CLI launchers, and M6 desktop UI remain planned. See [engine status](engine/README.md).
