...
This directory contains internal feature-plan JSON examples and Abstract Syntax Tree (AST) fixtures for CAD Copilot's pure parsing, validation, and lowering domain. The AI adapter and application execution path are planned, not implemented.
...
Geometry tests separately cover parsing, validation, and lowering. Application-level Example Plan loading and execution belong to M4; changes to these fixtures should be reviewed against current domain semantics.
