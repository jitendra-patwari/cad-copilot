...
**Status (10 September 2026):** Canonical wire schemas, manifest schemas, fixtures, typed immutable contract models, Draft 2020-12 validators, the operation metadata registry foundation (M5.1), sequential batch execution infrastructure (`BatchService`, typed bindings, work allocation, error isolation, cancellation, and observable teardown under M5.2), and batch filesystem & source-integrity safety boundary (`FilesystemBatchSafetyBoundary`, bounded root enforcement, streaming SHA-256 source snapshots, guarded workspaces, and Windows atomic no-replace publication under M5.3) are established, implemented, and verified under this directory and `engine/src/batch/`. Milestone 5.1, Milestone 5.2, and Milestone 5.3 are complete. Milestone 5.4+ format handlers, manifest publication, CLI launchers, and desktop client integration remain planned.
