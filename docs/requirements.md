...
- **Runbooks & Fixtures**: Internal feature-plan examples (`contracts/examples/`) cover plates, conceptual gears, and conditional composition/sweep representations. JSON fixture integrity and public schema conformance tests do not establish end-to-end or live CAD support; the generation runbook describes the implemented M4.5 stdio IPC launcher, whereas `contracts/schemas/batch/README.md` serves as the canonical contract guide for the batch interface (M5.1 through M5.6 implemented and verified, including contracts, execution infrastructure, safety boundary, genuine format handlers, Parasolid promotion, summary manifest publication, strict batch stdio transport, and launchers; Milestone 6 desktop client integration remains planned).
