...
- **Runbooks & Fixtures**: Internal feature-plan examples (`contracts/examples/`) cover plates, conceptual gears, and conditional composition/sweep representations. JSON fixture integrity and public schema conformance tests do not establish end-to-end or live CAD support; the generation runbook describes the implemented M4.5 stdio IPC launcher, whereas `contracts/schemas/batch/README.md` serves as the canonical contract guide for the batch interface (M5.1 contracts/schemas/models/registry, M5.2 sequential batch execution infrastructure, M5.3 filesystem safety boundary, and M5.4 genuine batch format handlers implemented; Milestone 5.5 conditional Parasolid export gate, Milestone 5.6 summary manifest publication, strict batch transport, and packaging, and desktop client integration remain planned).
