...
- **Runbooks & Fixtures**: Internal feature-plan examples (`contracts/examples/`) cover plates, conceptual gears, and conditional composition/sweep representations. JSON fixture integrity and public schema conformance tests do not establish end-to-end or live CAD support; the generation runbook describes the implemented M4.5 stdio IPC launcher, whereas `contracts/schemas/batch/README.md` serves as the canonical contract guide for the batch interface (M5.1 through M5.6 implemented and verified; M6.3 desktop client integration and M6.4 desktop packaging and installed acceptance fully implemented and verified; M6.5 CI gates verified green; M6.6 publication in progress).
...
- **Bounded Invocation & No Model Fallback**: At most two proposal attempts: the initial response and one regeneration only when local proposal validation reports `response_invalid`. Provider request, configuration, and empty-response failures do not trigger regeneration. No other repair loop or model fallback is used; every candidate passes the same canonical validation before CAD execution.
...
5. **Test-Harness Truthfulness & Native Gate Boundary (QA-08, SEC-07)**:
   ...
   - The historical M6.4 installed acceptance cases do not qualify a future installer containing the Gemini changes; that build requires its own installed acceptance.
